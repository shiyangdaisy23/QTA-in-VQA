...
def get_img(imgs):
    ...
    #img_pos = np.zeros(N, dtype='uint32')
    for img in imgs:
        count_img.append(img['img_path'])
        #count_img[] = count_img.get(img['img_path'], 0) + 1
...
def main(params):
    ...
    img_list_train = get_img(imgs_train)
    img_list_val = get_img(imgs_val)
    img_list_test = get_img(imgs_test)
    ...
    f.create_dataset("answers_train", dtype='uint32', data=A)
    ...
    json.dump(img_list_train, open('train_img_list.json', 'w'))
    json.dump(img_list_val, open('val_img_list.json', 'w'))
    json.dump(img_list_test, open('test_img_list.json', 'w'))
